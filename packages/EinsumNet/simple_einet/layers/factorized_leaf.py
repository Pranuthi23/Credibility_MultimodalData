...
from packages.EinsumNet.simple_einet.abstract_layers import AbstractLayer
from packages.EinsumNet.simple_einet.layers.distributions import AbstractLeaf
from packages.EinsumNet.simple_einet.sampling_utils import SamplingContext, index_one_hot
...
class FactorizedLeaf(AbstractLayer):
    ...
    def forward(self, x: torch.Tensor, marginalized_scopes: List[int], context=None):
        """
...
class ConditionalFactorizedLeaf(FactorizedLeaf):
    """
    A 'meta'-leaf layer that combines multiple scopes of a base-leaf layer via naive factorization.
    ...
    def forward(self, x: torch.Tensor, marginalized_scopes: List[int], context=None):
        ...
        """
        # Forward through base leaf
        x = self.base_leaf(x, marginalized_scopes, context)
