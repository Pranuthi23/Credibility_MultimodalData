...
from spn.distributions import Normal, dist_forward
from spn.distributions import IsotropicMultivariateNormal
from spn.distributions import MultivariateNormal
from spn.distributions import Leaf
from spn.layers import Product, Sum
...
class RegionSpn(nn.Module):
    ...
    def __init__(self, C, S, I, dropout, num_parts, num_recursions, in_features):
        """
    ...
    def _build(self):
        ...
        self._leaf = IndependentDirichlet(
            num_classes=self.C,
            multiplicity=self.I,
...
class RatSpnConstructor:
    ...
    def _create_spn(self, num_parts, num_recursions=1):
        ...
        spn = RegionSpn(
            self.C, self.S, self.I, self.dropout, num_parts, num_recursions, self.in_features
        )
...
class RatDirichlet(Leaf):
    """ 
    Dirichlet leaf layer. Maps each input feature to its dirichlet log likelihood."""
    ...
    def __init__(
        ...
        """
        super().__init__(multiplicity, in_features, dropout)
        # Create concentration param
        self.concentration = nn.Parameter(torch.randn(1, in_features, multiplicity, num_classes))
        
    def forward(self, x):
        dirichlet = dist.Dirichlet(concentration=torch.nn.functional.softmax(self.concentration,dim=-1))
        x = dist_forward(dirichlet, x)
        x = super().forward(x)
        return x
...
class IndependentDirichlet(Leaf):
    def __init__(self, num_classes, multiplicity, in_features, cardinality, dropout=0.0):
        """
        Create multivariate dirichlet
        ...
        Args:
            ...
            droptout: Dropout probabilities.
        """
        super(IndependentDirichlet, self).__init__(multiplicity, in_features, dropout)
        self.dirichlet = RatDirichlet(
            num_classes=num_classes, multiplicity=multiplicity, in_features=in_features, dropout=dropout
        )
        self.prod = Product(in_features=in_features, cardinality=cardinality)
        self.cardinality = cardinality
        self.out_shape = f"(N, {self.prod._out_features}, {multiplicity})"
    ...
    def __repr__(self):
        return f"IndependentDirichlet(in_features={self.in_features}, multiplicity={self.multiplicity}, dropout={self.dropout}, cardinality={self.cardinality}, out_shape={self.out_shape})"
