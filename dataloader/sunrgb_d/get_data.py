...
def get_dataloader(data_dir,FINE_SIZE, LOAD_SIZE, batch_size=40, num_workers=8, train_shuffle=True):
    ...
    train_data, val_data = random_split(AlignedConcDataset(FINE_SIZE = FINE_SIZE, LOAD_SIZE = LOAD_SIZE, data_dir=os.path.join(data_dir, 'train'), 
                                                           transform=transforms.Compose(train_transforms)), [0.8, 0.2])
    test_data = AlignedConcDataset(FINE_SIZE = FINE_SIZE, LOAD_SIZE = LOAD_SIZE, data_dir=os.path.join(data_dir, 'test'), 
